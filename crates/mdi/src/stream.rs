...
//! - `group_by_in_place_serial(Fn(&mut Vec<T>) -> Option<()>, grouping_fields: &[&str])`
//! - `group_by_in_place_parallel(Fn(&mut Vec<T>) -> Option<()>, grouping_fields: &[&str], n_cpu: usize, buffer_size: usize)`
//! - `group_by_replace_serial(Fn(&Vec<I>) -> Option<Vec<O>>, grouping_fields: &[&str])`
//! - `group_by_replace_parallel(Fn(&Vec<I>) -> Option<Vec<O>>, grouping_fields: &[&str], n_cpu: usize, buffer_size: usize)`
...
impl RecordStreamer {
    ...
    code notes:
        unwrap_or_else evaluates lazily
        enumerate starts at index 0, not 1
        all error handling:
            is done by the streaming functions, not the caller
            panics with the line number, since streaming errors are critical failures
    ------------------------------------------------------------------ */
    ...
    where
        ...
        F: Fn(&mut T) -> Option<()>,
    {
        const ERR: &str = "mdi::stream::RecordStreamer::stream_in_place_serial()";
        let (mut rdr, mut wtr) = init_io_streams(self, None);
        for (i, line) in rdr.deserialize().enumerate() {
            let mut input_record: T = line.unwrap_or_else(|e| {
                panic!("{ERR} failed while parsing input line {}: {e}", i + 1)
            });
            if let Some(_) = record_parser(&mut input_record) {
                wtr.serialize(input_record).unwrap_or_else(|e| {
                    panic!("{ERR} failed while processing input line {}: {e}", i + 1)
                });
            }
        }
        flush_stream(&mut wtr, ERR);
    }
    ...
    {
        const ERR: &str = "mdi::stream::RecordStreamer::stream_in_place_parallel()";
        let (mut rdr, mut wtr) = init_io_streams(self, Some(n_cpu));
        let mut input_record_buffer: Vec<T> = Vec::with_capacity(buffer_size);
        for (i, line) in rdr.deserialize().enumerate() {
            let input_record: T = line.unwrap_or_else(|e| {
                panic!("{ERR} failed while parsing input line {}: {e}", i + 1)
            });
            input_record_buffer.push(input_record);
            if input_record_buffer.len() == buffer_size {
                do_stream_in_place_parallel(&mut wtr, &mut input_record_buffer, &record_parser).unwrap_or_else(|e| {
                    panic!("{ERR} failed while processing buffer near input line {}: {e}", i + 1)
                });
                input_record_buffer.clear();
            }
        }
        do_stream_in_place_parallel(&mut wtr, &mut input_record_buffer, &record_parser).unwrap_or_else(|e| {
            panic!("{ERR} failed while processing the last buffered chunk: {e}")
        });
        flush_stream(&mut wtr, ERR);
    }
    ...
    where
        ...
        F: Fn(&I) -> Option<Vec<O>>,
    {
        const ERR: &str = "mdi::stream::RecordStreamer::stream_replace_serial()";
        let (mut rdr, mut wtr) = init_io_streams(self, None);
        for (i, line) in rdr.deserialize().enumerate() {
    ...
    {
        const ERR: &str = "mdi::stream::RecordStreamer::stream_replace_parallel()";
        let (mut rdr, mut wtr) = init_io_streams(self, Some(n_cpu));
        ...
        for (i, line) in rdr.deserialize().enumerate() {
            ...
            input_record_buffer.push(input_record);
            if input_record_buffer.len() == buffer_size {
                do_stream_replace_parallel(&mut wtr, &input_record_buffer, &record_parser).unwrap_or_else(|e| {
                    panic!("{ERR} failed while processing buffer near input line {}: {e}", i + 1)
                });
                input_record_buffer.clear();
            }
    ...
    {
        const ERR: &str = "mdi::stream::RecordStreamer::group_by_in_place_serial()";
        let (mut rdr, mut wtr) = init_io_streams(self, None);
        ...
        for (i, line) in rdr.deserialize().enumerate() {
            ...
            let this_key = get_composite_key(&input_record, grouping_fields).unwrap_or_else(|e| {
                panic!("{ERR} failed to get group key from line {}: {e}", i + 1)
            });
            if previous_key.as_ref().map_or(false, |k| k != &this_key) {
                do_group_by_in_place_serial(&mut wtr, &mut input_record_group, &record_parser).unwrap_or_else(|e| {
                    panic!("{ERR} failed while processing group ending at line {}: {e}", i)
                });
        ...
        do_group_by_in_place_serial(&mut wtr, &mut input_record_group, &record_parser).unwrap_or_else(|e| {
            panic!("{ERR} failed while processing the last group: {e}")
        });
        flush_stream(&mut wtr, ERR);
    }
    ...
    {
        const ERR: &str = "mdi::stream::RecordStreamer::group_by_in_place_parallel()";
        let (mut rdr, mut wtr) = init_io_streams(self, Some(n_cpu));
        ...
        for (i, line) in rdr.deserialize().enumerate() {
            ...
            if previous_key.as_ref().map_or(false, |k| k != &this_key) {
                ...
                if input_record_group_buffer.len() == buffer_size {
                    do_group_by_in_place_parallel(&mut wtr, &mut input_record_group_buffer, &record_parser).unwrap_or_else(|e| {
                        panic!("{ERR} failed while processing group ending at line {}: {e}", i)
                    });
    ...
    {
        const ERR: &str = "mdi::stream::RecordStreamer::group_by_replace_serial()";
        let (mut rdr, mut wtr) = init_io_streams(self, None);
        ...
        for (i, line) in rdr.deserialize().enumerate() {
            ...
            if previous_key.as_ref().map_or(false, |k| k != &this_key) {
                do_group_by_replace_serial(&mut wtr, &input_record_group, &record_parser).unwrap_or_else(|e| {
                    panic!("{ERR} failed while processing group ending at line {}: {e}", i)
                });
                input_record_group.clear();
            }
    ...
    {
        const ERR: &str = "mdi::stream::RecordStreamer::group_by_replace_parallel()";
        let (mut rdr, mut wtr) = init_io_streams(self, Some(n_cpu));
        ...
        for (i, line) in rdr.deserialize().enumerate() {
            ...
            if previous_key.as_ref().map_or(false, |k| k != &this_key) {
                ...
                input_record_group = Vec::new(); // prevents move error with input_record_group.clear(); must reallocate
                if input_record_group_buffer.len() == buffer_size {
                    do_group_by_replace_parallel(&mut wtr, &input_record_group_buffer, &record_parser).unwrap_or_else(|e| {
                        panic!("{ERR} failed while processing group buffer ending at input line {}: {e}", i)
                    });
                    input_record_group_buffer.clear();
                }
            }
...
fn get_composite_key<T>(record: &T, grouping_fields: &[&str]) -> Result<String, Box<dyn Error>> 
where 
    T: Serialize,
{
    if grouping_fields.len() == 1 {
        get_field_value(record, grouping_fields[0])
    } else {
        grouping_fields
            .iter()
            .map(|&grouping_field| get_field_value(record, grouping_field))
...
fn flush_stream(wtr: &mut csv::Writer<std::io::Stdout>, err: &str) {
    wtr.flush().expect(&format!("{err} failed while flushing last records in output stream"));
}
